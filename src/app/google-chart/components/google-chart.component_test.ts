...
describe('ChartComponent', () => {
  ...
  const findInChildren = (
    parent: HTMLElement,
    comparison: (el: HTMLElement) => boolean
  ): HTMLElement | null => {
    const children = Array.from(parent.children);
    for (const child of children) {
      if (comparison(child as HTMLElement)) {
        return child as HTMLElement;
      } else {
        const found = findInChildren(child as HTMLElement, comparison);
        if (found) {
          return found;
        }
      }
  ...
  describe('Generic Chart tests', () => {
    ...
    it('should match parent width', () => {
      ...
      const chartContainer = chartElement.parentElement as HTMLElement;
      chartContainer.style.width = '100%';

      component.ngOnChanges();

      const chartParent = chartContainer.parentElement as HTMLElement;
      if (chartParent) {
